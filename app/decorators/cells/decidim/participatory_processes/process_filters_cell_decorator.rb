  ...
  def process_count_by_filter
    if current_user&.admin?
      ...
      @process_count_by_filter = %w(active upcoming past).inject({}) do |collection_by_filter, filter_name|
        ...
        filtered_ids = filtered_processes.present? ? filter_by_suara_permissions(filtered_processes(filter_name)).map(&:decidim_participatory_process_group_id) : []
        processes = filtered_processes.groupless.present? ? filter_by_suara_permissions(filtered_processes(filter_name).groupless) : []
