  ...
  class Permissions < DefaultPermissions
    ...
    def component_public_action?
      ...
      if component.published?
        if component.participatory_space.scope.present?
          return allow! if user_authorization_contains_scope?
        else
          return allow!
        end
      end
    ...
    def user_authorization_contains_scope?
      return unless user
      return unless authorization = Decidim::Authorization.find_by(decidim_user_id: user.id)

      allow = false
      authorization.metadata.values.flatten.each do |field|
        allow = true if field == component.participatory_space.scope.name[I18n.locale.to_s]
      end
      allow
    end
  end
